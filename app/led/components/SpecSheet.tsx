...
/**
 * On-screen preview of the spec sheet. Mirrors the print layout
 * (Section /05 Artifact A): header strip, project block, 4-column hero,
 * context strip, footer. PDF generation lives in Phase 2.
 */
  ...
  return (
    <article className="panel" id="spec-sheet">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 border-b hairline">
        <Hero
          label="TILES"
          big={`${cfg.tiles_wide}×${cfg.tiles_high}`}
          sub={`${fmt.int(d.tiles_total)} TOTAL`}
        />
        <Hero
          label="PIXELS"
          big={`${fmt.int(d.pixels_wide)}×${fmt.int(d.pixels_high)}`}
          sub={`${fmt.int(d.pixels_total)} TOTAL`}
        />
        <Hero
          label="WALL"
          big={`${d.wall_width_m.toFixed(2)}×${d.wall_height_m.toFixed(2)} m`}
          sub={`${d.wall_width_imperial} × ${d.wall_height_imperial}`}
        />
        <Hero
          label="POWER"
          big={`${fmt.num(d.amps_max_per_phase, 0)} A`}
          sub={`MAX / ${fmt.num(d.amps_avg_per_phase, 0)} A AVG / ${cfg.power_service}`}
        />
      </div>

      {/* CONTEXT GRID */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-x-5 gap-y-3 px-5 py-4 border-b hairline">
...
function Hero({ label, big, sub }: { label: string; big: string; sub: string }) {
  return (
    <div className="px-5 py-5 border-r hairline last:border-r-0">
      ...
      <div className="mono text-[22px] font-bold accent leading-none tracking-tight tabular-nums">
        {big}
      </div>
...
function KV({
  ...
  return (
    <div className={wide ? "md:col-span-2" : undefined}>
      ...
      <div className="mono text-[13px] font-bold mt-0.5 tabular-nums">{value}</div>
    </div>
  )
}
